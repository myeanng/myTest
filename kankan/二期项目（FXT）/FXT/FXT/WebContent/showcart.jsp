...
<!-- form是显示购物车内容部分 -->
<form action="AddOrdersServlet?xuan=1" id="myform" method="post">
<div id="four4">每人最多只能选购100件商品</div>
<!-- five是显示购物车内容列表部分 -->
<div id="five">
<table border="1" style="border-collapse:collapse;width:979px;text-align:center;">
	<tr style="background-color:#F7F7F7;font-weight:bold;">
	    <td><input type="checkbox" name="xuanze" id="all" value="0" onclick="qx()">全选</td>
	    <td>图片预览</td>
	    <td>商品名称</td>
        <td>数量</td>
        <td>总价</td>
...
<c:choose>
     <c:when test="${!empty nowuser }">
          <c:choose>
               ...
               <tr><td colspan="6">亲！购物车还是空的哦！</td></tr>
               </c:when>
               <c:otherwise>
                    <c:forEach items="${cartlist }" var="c">
                    <tr id="${c.gid }"><td ><input type="checkbox" name="xuanze" value="${c.gid }"></td><td><img src="img/${c.gpic1 }" style="width:40px;height:40px;"></td><td>${c.gname }</td><td><input type="button" value="-" class="jian" ><input type="text" value="${c.num }" style="width:50px;" class="num" maxlength="2"><input type="button" value="+" class="jia"></td><td><span style="color:red;font-size:15px">￥</span><input type="text" class="zongjia" value="${c.totalmoney }" style="width:50px;color:red;font-size:15px;border:none;" readonly="readonly"></td><td><a href="DelCartServlet?id=${c.gid }">删除</a></td></tr>
                    </c:forEach>
               </c:otherwise>
          </c:choose>
     </c:when>
     <c:otherwise>
          <c:choose>
               ...
               <c:otherwise>
                     ...
                     <tr id="${c.value.gid }"><td><input type="checkbox" name="xuanze"></td><td><img src="img/${c.value.gpic1 }" style="width:40px;height:40px;"></td><td>${c.value.gname }</td><td><input type="button" value="-" class="jian"><input type="text" value="${c.value.num }" style="width:50px;" class="num" maxlength="2"><input type="button" value="+" class="jia"></td><td><span style="color:red;font-size:15px">￥</span><input type="text" class="zongjia" value="${c.value.totalmoney }" style="width:50px;color:red;font-size:15px;border:none;" readonly="readonly"></td><td><a href="DelCartServlet?id=${c.value.gid }">删除</a></td></tr>
                     </c:forEach>
               </c:otherwise>
          </c:choose>
     </c:otherwise>
</c:choose>
<tr><td colspan=6 style="text-align:right;height:50px;"><span style="color:block;font-weight:bold;font-size:14px;">应付金额：</span><span style="color:red;font-weight:bold;font-size:18px">￥<input type="text" value="0" id="heji" name="zongjia" style="color:red;font-weight:bold;font-size:18px;width:50px;border:none;" readonly="readonly">元</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
</table>
</div>
<!-- six是显示收货地址部分 -->
<div id="six">
...
<!-- seven是提交订单按钮部分 -->
<div id="seven" style="text-align:right;">
...
$(".jian").click(function(){
	...
	var yuanzongjia=$(this).parent().parent().children().eq(4).find("input").val();
	var gid=$(this).parent().parent().get(0).id;
	$.ajax({
		...
		success:function(data){
			var zongjia=parseFloat(data);
			$("#"+gid).children().eq(4).find("input").val(parseFloat(zongjia));
			var chajia=zongjia-yuanzongjia;
			if($("#"+gid).children().eq(0).find("input").get(0).checked){
				var yuanheji=parseFloat($("#heji").val());
				$("#heji").val(yuanheji+chajia);
			};
		},
...
//单击每个复选框时动态改变合计的价格
$("#myform :checkbox:gt(0)").click(function(){
	var num=0;
	var heji=parseFloat($("#heji").val());
	var xuan=$(this).get(0).checked;
	var zongjia=parseFloat($(this).parent().parent().children().eq(4).find("input").val());
	if(xuan==true){
		heji=heji+zongjia;
		$("#heji").val(heji);
	}else{
	...
	if(${!empty nowuser.uid}){
		$(":checked").each(function(){
			var gid=parseInt($(this).parent().parent().get(0).id);
			$.ajax({
				url:'QueryGclidByGidServlet?abc='+Math.random(),
				type:'post',
				dataType:'json',
				data:{
					gid:gid
...
//控制页面是否显示提交订单按钮
$(function(){
	if(${nowuser==null}&&${empty carts}){
		$("#myform").css("display","none");
		$("#four3").css({"height":"340px","border":"1px solid #f5f5f5","background":"url(pic/cartnull.jpg)"});
	}
	if(${nowuser!=null}&&${empty cartlist}){
		$("#myform").css("display","none");
		$("#four3").css({"height":"340px","border":"1px solid #f5f5f5","background":"url(pic/cartnull.jpg)"});
	}
})
