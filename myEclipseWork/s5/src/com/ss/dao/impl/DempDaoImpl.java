package com.ss.dao.impl;

import java.util.List;

import org.hibernate.SessionFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.orm.hibernate3.support.HibernateDaoSupport;
import org.springframework.stereotype.Repository;

import com.ss.dao.DempDao;
import com.ss.entity.Demp;
@Repository("dd")
public class DempDaoImpl extends HibernateDaoSupport implements DempDao{

	@Autowired
	public void MySetSessionFactory(  SessionFactory sf){
		super.setSessionFactory(sf);
	}
	
	public void addDemp(Demp demp) {
		// TODO Auto-generated method stub
		this.getHibernateTemplate().save(demp);
	}

	public void deleteDemp(int id) {
		// TODO Auto-generated method stub
		
	}

	public List<Demp> findAllDemp() {
		// TODO Auto-generated method stub
		return this.getHibernateTemplate().find("from Demp");
	}

	public List<Demp> findAllDempByPage(int page, int size) {
		// TODO Auto-generated method stub
		return null;
	}

	public void updateDemp(Demp demp) {
		// TODO Auto-generated method stub
		
	}

}
