			...
			<tr>
				<td>
					<strong class="fl w"> <img
							...
							<a href="#category.id=${ board.category.id }">${board.category.name }</a> &raquo; 
								<a href="#board.id=${ board.id }">${board.name }</a> 
							</strong>
					<div class="fr w">
						<a href="#board.id=${ board.id }">版主</a>:
						&nbsp;
						<c:forEach items="${ board.administrators }" var="person">
							<a
								href="#person.id=${ person.id }">${person.account }</a>
						</c:forEach>
					</div>
				</td>
			</tr>
				...
				<c:forEach var="thread" items="${ threadList }">
					...
					<tr align="center" class="tr3 t_one">
						...
						<td class="tal y-style">
							<a href="#person.id=${ thread.author.id }" class="bl">${ thread.author.account }</a>
							<div class="f10">
								${ thread.dateCreated }
							</div>
						</td>
						...
						<td class="tal y-style">
							...
							<c:if test="${ thread.authorLastReplied != null }">
								<a href="#person.id=${ thread.authorLastReplied.id }" class="bl">${ thread.authorLastReplied.account }</a>
								<div class="f10">
		...
		<table cellspacing="0" cellpadding="0" width="100%" align="center">
			<tr valign="top">
				...
				<td style="text-align:right;padding-top:5px">
					<a
						href="#"><img src="images/yellow/post.png" id="td_post" /> </a>
				</td>
			</tr>
		</table>
	</div>
