    ...
    <ul class="breadcrumb" style="border: none !important;">
        <li><a href="http://www.ppdai.com/">首页</a> <span class="divider">&gt;</span></li>
            <li><a href="myAccount-baccount">我的账户-我是借款者</a> <span class="divider">&gt;</span></li>
                    <li><a href="myAccount-myloan2">已发布的列表</a></li>
    </ul>
</div>
<script>
        ...
        <div class="my-f-right fr">
            <div class="lendtotal_nav " style="margin-top: 0;">
                <div class="my-f-r-ppb clearfix">
                    <div class="outed fl"><a href="myAccount-myloan">正在还款的列表</a></div>
                    ...
                    <tr>
                       ...
                       <tr><td><fmt:formatDate value="${l.createtime}" pattern="yyyy-MM-dd hh:mm"/></td>
                       	   <td>${l.title}</td>
	                        <td>¥${l.loanmoney}</td>
	                        <td>${l.loantime}个月</td>
	                        <td>${l.rate}%</td>
	                        <td>
	                        <c:if test="${l.status==1}">审核中</c:if>
	                        <c:if test="${l.status==3}">审核中</c:if>
	                        <c:if test="${l.status==4}">融资中</c:if>
	                        <c:if test="${l.status==6}">融资完成</c:if>
	                        <c:if test="${l.status==7}">还款中</c:if>
	                        <c:if test="${l.status==8}">已完结</c:if>
	                        <c:if test="${l.status==9}">逾期项目</c:if>
	                        </td>
                       </tr>
