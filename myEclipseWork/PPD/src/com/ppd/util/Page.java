...
public class Page {
	...
	private long pageIndex = 0;					// 当前页是从第几条数据开始查询
	...
	private void init(int total, int pageNumber, int limit){
		...
		this.pageIndex = (this.pageNumber-1) * limit;
	...
	public String toString(){
		...
		sb.append("[")
			.append("总记录数=").append(total)
			.append(",总页数=").append(pages)
			.append(",当前页=").append(pageNumber)
			.append(",当前页从第").append(pageIndex).append("条数据开始查询")
			.append(",每页显示记录数=").append(limit)
			.append(",是否为第一页=").append(isFirstPage)
			.append(",是否为最后一页=").append(isLastPage)
			.append(",是否有前一页=").append(hasPreviousPage)
			.append(",是否有下一页=").append(hasNextPage)
		.append(",所有导航页号=");
